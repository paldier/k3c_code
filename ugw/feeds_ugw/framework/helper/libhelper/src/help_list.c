...
OUT ObjAttrList *  help_addObjAttrList(IN ObjAttrList *pxObjList, IN const char *pcObjName, IN const char *pcWebName,IN uint32_t unFlag)           
{
	...
	if(pcObjName != NULL)
	{
		strncpy(pxTmp->sObjName,pcObjName,MAX_LEN_OBJNAME);
		pxTmp->sObjName[MAX_LEN_OBJNAME-1]='\0';
	}

	if(pcWebName != NULL)
	{
		strncpy(pxTmp->sObjWebName,pcWebName,MAX_LEN_WEBNAME);
		pxTmp->sObjWebName[MAX_LEN_WEBNAME-1]='\0';
	}
			  ...
			  IN uint32_t unParamFlag)
{
	...
	if(pcParamName != NULL)
	{
		strncpy(pxTmp->sParamName,pcParamName,MAX_LEN_PARAM_NAME);
		pxTmp->sParamName[MAX_LEN_PARAM_NAME-1]='\0';
	}
	
	if(pcParamProfile != NULL)
	{
		strncpy(pxTmp->sParamProfile,pcParamProfile,MAX_LEN_PROFILENAME);
		pxTmp->sParamProfile[MAX_LEN_PROFILENAME-1]='\0';
	}
	
	if(pcParamWebName != NULL)
	{
		strncpy(pxTmp->sParamWebName,pcParamWebName,MAX_LEN_WEBNAME);
		pxTmp->sParamWebName[MAX_LEN_WEBNAME-1]='\0';
	}
	
	if(pcValidVal != NULL)
	{
		strncpy(pxTmp->sParamValidVal,pcValidVal,MAX_LEN_VALID_VALUE);
		pxTmp->sParamValidVal[MAX_LEN_VALID_VALUE-1]='\0';
	}
	
	if(pcParamValue != NULL)
	{
		strncpy(pxTmp->sParamValue,pcParamValue,MAX_LEN_PARAM_VALUE);
		pxTmp->sParamValue[MAX_LEN_PARAM_VALUE-1]='\0';
	}
...
void help_printObj(IN void *pxObj,uint32_t unSubOper)
{
	if (IS_OBJLIST(unSubOper))
	{
		...
		{
			nCnt = strlen(GET_OBJ_NAME(pxTmpObj))+strlen(sBuf)+16;
			sBuf = HELP_REALLOC(sBuf,nCnt);
			if(sBuf == NULL)
				goto finish;

			snprintf(sBuf,nCnt,"%s \t \"%s\": { \n",sBuf,GET_OBJ_NAME(pxTmpObj));

			sTmp[0]='\0';
			sprintf(sTmp,"%d", GET_OBJ_SID(pxTmpObj));	

			nCnt = strlen(sBuf)+strlen(sTmp)+24;
			sBuf = HELP_REALLOC(sBuf,nCnt);
			if(sBuf == NULL)
				goto finish;
			 
			snprintf(sBuf, nCnt, "%s \t\t \"Sid\" :\"%d\",\n",sBuf, GET_OBJ_SID(pxTmpObj));

			sTmp[0]='\0';
			sprintf(sTmp,"%d", GET_OBJ_OID(pxTmpObj));	

			nCnt = strlen(sTmp)+strlen(sBuf)+26;
			sBuf = HELP_REALLOC(sBuf,nCnt);
			if(sBuf == NULL)
				goto finish;

			snprintf(sBuf, nCnt, "%s \t\t \"Oid\" : \"%d\",\n",sBuf, GET_OBJ_OID(pxTmpObj));

			if (IS_SOPT_OBJNAME(unSubOper))
			{
				sTmp[0]='\0';
				sprintf(sTmp,"%d", GET_OBJ_SUBOPER(pxTmpObj));	

				nCnt = strlen(sTmp)+strlen(sBuf)+30;
				sBuf = HELP_REALLOC(sBuf,nCnt);
				if(sBuf == NULL)
					goto finish;

				snprintf(sBuf,nCnt, "%s \t\t \"ObjOper\" : \"%d\"\n",sBuf, GET_OBJ_SUBOPER(pxTmpObj));
			}
			else
			{
				sTmp[0]='\0';
			...
			sTmp[0]='\0';
			sprintf(sTmp,"%d", GET_OBJ_FLAG(pxTmpObj));	
			...
			snprintf(sBuf, nCnt, "%s \t\t \"Flag\" : \"0x%x\",\n",sBuf, GET_OBJ_FLAG(pxTmpObj));

			FOR_EACH_PARAM(pxTmpObj,pxParam)
			{
				
				nCnt  = strlen(GET_PARAM_NAME(pxParam))+strlen(sBuf)+22;	
				sBuf = HELP_REALLOC(sBuf, nCnt);
				if(sBuf == NULL)
					goto finish;

				snprintf(sBuf, nCnt, "%s \t\t \"%s\": { \n",sBuf, GET_PARAM_NAME(pxParam));

				sTmp[0]='\0';
				sprintf(sTmp,"%d", GET_PARAM_ID(pxParam));	
				
				nCnt = strlen(sTmp)+strlen(sBuf)+38;
				sBuf = HELP_REALLOC(sBuf,nCnt);
				if(sBuf == NULL)
					goto finish;

				snprintf(sBuf, nCnt, "%s \t\t\t \"paramId\" : \"%d\",\n",sBuf, GET_PARAM_ID(pxParam)); 

				nCnt = strlen(GET_PARAM_VALUE(pxParam))+strlen(sBuf)+40;
				sBuf = HELP_REALLOC(sBuf,nCnt);
				if(sBuf == NULL)
					goto finish;
	
				snprintf(sBuf, nCnt, "%s \t\t\t \"paramValue\" : \"%s\",\n",sBuf, GET_PARAM_VALUE(pxParam)); 

				sTmp[0]='\0';
				sprintf(sTmp,"%d", GET_PARAM_FLAG(pxParam));
				nCnt = strlen(sTmp)+strlen(sBuf)+40;
				sBuf = HELP_REALLOC(sBuf,nCnt);
				if(sBuf == NULL)
					goto finish;

				snprintf(sBuf, nCnt, "%s \t\t\t \"paramFlag\" : \"0x%x\"\n",sBuf, GET_PARAM_FLAG(pxParam)); 
			...
			FOR_EACH_PARAM(pxTmpObjDst,pxParam)
			{
				if ( (strcmp(pxParam->sParamName,pcParamName) == 0 ) && (strlen(pxParam->sParamName) == strlen(pcParamName)))
				{
					if(pcParamValue != NULL)
					{
						...
						{
							strncpy(pxParam->sParamValue,pcParamValue,MAX_LEN_PARAM_VALUE);
							pxParam->sParamValue[MAX_LEN_PARAM_VALUE-1]='\0';
							nRet = UGW_SUCCESS;	
						}
						else
						{
		...
		FOR_EACH_OBJ(pxObj, pxTmpObj)
		{
			if(pxTmpObj->unOid == unOid) 
			{
				FOR_EACH_PARAM(pxTmpObj, pxParam)
				{
					...
					{
						strncpy(pcVal, pxParam->sParamValue, MAX_LEN_PARAM_VALUE);
						pcVal[MAX_LEN_PARAM_VALUE-1]='\0';
						nRet = UGW_SUCCESS;
						return nRet;
					}
				}
				...
				FOR_EACH_PARAM(pxTmpObj, pxParam)
				{
					if (pxParam->unParamId == unParamId) 
					{
						if(pcVal != NULL)
						{
							...
							{
								strncpy(pxParam->sParamValue, pcVal , MAX_LEN_PARAM_VALUE);
								pxParam->sParamValue[MAX_LEN_PARAM_VALUE-1]='\0';
								nRet=UGW_SUCCESS;
							}
							else
