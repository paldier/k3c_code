...
# in future revisions by a hook that calls a function from fapi_wlan_wave_init_platform.sh
pbc_init () {
	PBC_NAME=$1
	PBC_TRIGGER=$2
	
	# Insmod the wps_pbc driver for polling gpios
	if [ -e /usr/drivers/wps_pb.ko ] && [ `lsmod | grep wps_pb -c` -eq 0 ]; then
...
usage () {
	echo USAGE:
	echo "fapi_wlan_wave_led_control.sh <command> <led_name/pbc_name> <options>"
	echo Commands:
	echo -e "    led_on\n    led_off\n    led_blink\n    led_netif_activity_trigger\n    pbc_init\n    pbc_wait"
	echo Options:
