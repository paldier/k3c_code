...
restart_inetd()
{
    if [ `ps | grep -c inetd` -gt 1 ] && [ `grep -c tftpd /opt/lantiq/etc/inetd.conf` -gt 0 ]
    then
        grep -v tftpd /opt/lantiq/etc/inetd.conf > /tmp/inetd.conf
        killall inetd
        /opt/lantiq/usr/sbin/inetd /tmp/inetd.conf
    fi
}
