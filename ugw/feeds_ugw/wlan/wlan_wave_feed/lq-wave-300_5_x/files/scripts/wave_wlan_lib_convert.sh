...
convert_power_coc_auto_config()
{
	# Define local parameters
	local ap_index
	local pcoc_interval pcoc_low_threshold pcoc_upper_threshold

	ap_index=$1

	# Return the interval to check the traffic, the low and high thresholds for the Power CoC(CPU frequecy OTF).
	eval pcoc_interval=\${wlcoc_${ap_index}_pCocIntvl}
	eval pcoc_low_threshold=\${wlcoc_${ap_index}_pCocLowThreshold}
	eval pcoc_upper_threshold=\${wlcoc_${ap_index}_pCocUpperThreshold}
	echo "${pcoc_interval} ${pcoc_low_threshold} ${pcoc_upper_threshold}"
}
