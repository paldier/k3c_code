...
local sReliableMcast sIpxPpaEnabled
...
# Set hairpin mode to the VAP if radio is in fastpath
if [ "$fastpath_enabled" = "1" ]
then
	# If hairpin_mode is already enabled, no need to set it again.
	hairpin_mode="0"
	[ -e /sys/class/net/br0/brif/${interface_name}/hairpin_mode ] && hairpin_mode=`cat /sys/class/net/br0/brif/${interface_name}/hairpin_mode`
	[ "$hairpin_mode" -ne "1" ] && set_conf_param drv_config_post_up proc otf $pid $interface_name "/sys/class/net/br0/brif/${interface_name}/hairpin_mode" "1"
fi
