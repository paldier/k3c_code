...
sScanParams sScanParamsBG sScanModifFlags sScanCalCwMasks sFWRecovery
local sCoCPower sEnableRadio
...
# TODO: remove sCoCPower dependency when pcoc parameters won't change after disabling radio.
# TODO: enable PCoCPower configurations once it is supported by the driver.
#[ "$sCoCPower" = "1" ] && [ "$sPCoCPower_changed" ] && set_conf_param drv_config_pre_up iwpriv no_otf $pid $interface_name sPCoCPower "$sPCoCPower"
#[ "$sCoCPower" = "1" ] && [ "$sPCoCAutoCfg_changed" ] && set_conf_param drv_config_pre_up iwpriv no_otf $pid $interface_name sPCoCAutoCfg "$sPCoCAutoCfg"
